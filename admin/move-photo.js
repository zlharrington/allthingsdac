...
const destinations=[
 ...
 {page:'about',field:'davidImage',label:'About — David Peters photo'},
 {page:'about',field:'calebImage',label:'About — Caleb Peters photo'},
 {page:'about',field:'jonathanImage',label:'About — Jonathan Peters photo'}
];
...
function inject(card){if(card.querySelector('.move-site-image'))return;const body=card.querySelector('.admin-card-body');const actions=body?.querySelector('.admin-card-actions');if(!body||!actions)return;const box=document.createElement('div');box.className='move-site-image';box.innerHTML=`<span>Assign site image</span><div class="move-site-controls"><select class="move-site-destination" aria-label="Static website image destination">${options()}</select><button class="btn btn-outline move-site-photo" type="button">Assign</button></div><small class="move-site-help">Assigns this photo to a fixed GitHub site-image slot. The original photo stays in its current folder.</small><p class="move-site-error" role="alert" aria-live="assertive"></p>`;body.insertBefore(box,actions);}
...
grid.addEventListener('click',async event=>{const button=event.target.closest('.move-site-photo');if(!button)return;const card=button.closest('.admin-card');const select=card?.querySelector('.move-site-destination');clearMoveError(card);const selected=select?.value;if(!card||!selected){showMoveError(card,'Choose a destination before clicking Assign.');return;}const destination=destinations.find(item=>`${item.page}|${item.field}`===selected);if(!destination){showMoveError(card,'The selected destination is invalid.');return;}button.disabled=true;select.disabled=true;try{if(typeof setStatus==='function')setStatus(`Assigning photo to ${destination.label} in GitHub…`);const result=await jsonRequest('/api/admin/move-site-image',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({key:card.dataset.key,page:destination.page,field:destination.field})});if(typeof setStatus==='function')setStatus(`Assigned photo to ${destination.label}. GitHub commit ${String(result.commitSha||'').slice(0,7)} created; refreshing admin…`);window.location.reload();}catch(error){showMoveError(card,error.message);if(typeof setStatus==='function')setStatus(error.message,true);button.disabled=false;select.disabled=false;}});
}
enable();
})();
