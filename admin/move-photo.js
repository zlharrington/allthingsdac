...
function inject(card){if(card.querySelector('.move-site-image'))return;const body=card.querySelector('.admin-card-body');const actions=body?.querySelector('.admin-card-actions');if(!body||!actions)return;const box=document.createElement('div');box.className='move-site-image';box.innerHTML=`<span>Move to site image</span><div class="move-site-controls"><select class="move-site-destination" aria-label="Static website image destination">${options()}</select><button class="btn btn-outline move-site-photo" type="button">Move</button></div><small class="move-site-help">Replaces the selected static site image and removes this photo from its project gallery.</small>`;body.insertBefore(box,actions);}
...
async function enable(){let access=window.DAC_ADMIN_ACCESS;if(window.DAC_ADMIN_ACCESS_PROMISE)access=await window.DAC_ADMIN_ACCESS_PROMISE;const allowed=access?.role==='owner'||(access?.permissions?.gallery===true&&access?.permissions?.siteEditor===true);if(!allowed)return;injectAll();new MutationObserver(injectAll).observe(grid,{childList:true,subtree:true});
grid.addEventListener('click',async event=>{const button=event.target.closest('.move-site-photo');if(!button)return;const card=button.closest('.admin-card');const select=card?.querySelector('.move-site-destination');const selected=select?.value;if(!card||!selected)return;const destination=destinations.find(item=>`${item.page}|${item.field}`===selected);if(!destination)return;const title=card.querySelector('.edit-title')?.value.trim()||'This photo';if(!confirm(`Move "${title}" to ${destination.label}?\n\nThis will replace the current site image in that location and remove this photo from the project gallery.`))return;button.disabled=true;select.disabled=true;try{if(typeof setStatus==='function')setStatus(`Moving photo to ${destination.label}…`);await jsonRequest('/api/admin/move-site-image',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({key:card.dataset.key,page:destination.page,field:destination.field})});card.remove();if(typeof setStatus==='function')setStatus(`Moved photo to ${destination.label}.`);}catch(error){if(typeof setStatus==='function')setStatus(error.message,true);else alert(error.message);button.disabled=false;select.disabled=false;}});
}
enable();
})();
