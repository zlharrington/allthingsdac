...
const COMMERCIAL_CATEGORIES=new Set(['retail','medical','auto-dealerships','general']);
function cleanType(value){const type=String(value||'').trim().toLowerCase();return JOB_TYPES.has(type)?type:'';}
function cleanCommercialCategory(value){const category=String(value||'').trim().toLowerCase();return COMMERCIAL_CATEGORIES.has(category)?category:'general';}
...
export async function onRequestGet(context){
  ...
  const projects=projectObjects.map(object=>{const meta=object.customMetadata||{};const jobType=cleanType(meta.jobType)||'commercial';return{id:meta.id||object.key.slice('projects/'.length).replace(/\.json$/,''),name:meta.name||'Untitled Project',jobType,commercialCategory:jobType==='commercial'?cleanCommercialCategory(meta.commercialCategory):'',description:meta.description||'',createdAt:meta.createdAt||''};}).filter(project=>!requestedType||project.jobType===requestedType);
  const projectIds=new Set(projects.map(project=>project.id));
  const photos=photoObjects.map(object=>{const meta=object.customMetadata||{};if(meta.published!=='true')return null;const file=object.key.slice('gallery/'.length);return{file,url:`/media/${encodeURIComponent(file)}`,title:meta.title||'',alt:meta.alt||'All Things Drywall & Construction project photo',jobType:cleanType(meta.jobType),projectId:meta.projectId||'',featured:meta.featured==='true',createdAt:meta.createdAt||''};}).filter(photo=>photo&&projectIds.has(photo.projectId));
  photos.sort((a,b)=>(Number(b.featured)-Number(a.featured))||String(b.createdAt).localeCompare(String(a.createdAt)));
