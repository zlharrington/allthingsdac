...
const runtimeThemeStyles=document.createElement('style');runtimeThemeStyles.id='dac-runtime-theme';document.head.appendChild(runtimeThemeStyles);
async function loadRuntimeTheme(){try{const response=await fetch(`/api/site-content?page=global&theme=${Date.now()}`,{cache:'no-store'});if(!response.ok)return;const data=await response.json();const css=String(data?.content?._themeCss||'').trim();if(css)runtimeThemeStyles.textContent=css;}catch(error){console.warn('Runtime theme could not be loaded.',error);}}
loadRuntimeTheme();
