...
async function hydrate(){const key=currentPageKey();const page=pages[key];if(!page)return;try{const response=await fetch(`/api/site-content?page=${encodeURIComponent(key)}`);if(!response.ok)return;const data=await response.json();applyContent(page,data.content||{});}catch(error){console.warn('Page content overrides could not be loaded.',error);}}
