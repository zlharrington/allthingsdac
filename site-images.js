...
async function hydrateStaticSiteImages(){try{if(window.DAC_SITE_CONTENT_READY)await window.DAC_SITE_CONTENT_READY;const response=await fetch('/site-image-slots.json',{cache:'no-cache'});if(!response.ok)return;const slots=await response.json();const logo=String(slots['global.siteLogo']||'').trim();if(logo)document.querySelectorAll('.brand-logo img').forEach(img=>{img.src=logo;img.removeAttribute('srcset');});const page=currentPageKey();if(!page)return;const selectors=pageSelectors[page]||{};for(const [field,selector] of Object.entries(selectors)){const url=String(slots[`${page}.${field}`]||'').trim();if(url)applyImage(selector,url);}}catch(error){console.warn('Static site images could not be loaded.',error);}}
